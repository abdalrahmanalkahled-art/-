...
export function AppPageHeader({ title, subtitle, onBack, rightContent }: AppPageHeaderProps) {
  ...
  return (
    <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
      {onBack ? <TouchableOpacity onPress={onBack} accessibilityLabel="العودة" style={styles.iconButton}><MaterialIcons name="arrow-back" size={DESIGN.icon.standard} color={colors.foreground} /></TouchableOpacity> : <View style={styles.iconButton} />}
...
const styles = StyleSheet.create({
  ...
  copy: { flex: 1, alignItems: "flex-start", gap: 2 },
  title: { fontSize: 17, fontWeight: "800", textAlign: "left" },
  subtitle: { fontSize: 10, textAlign: "left" },
});
