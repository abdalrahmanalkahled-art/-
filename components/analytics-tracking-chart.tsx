...
type ChartTooltip = { title: string; detail: string; value: number; x: number; y: number };
...
export function AnalyticsTrackingChart({ analytics, scopeLabel, chartType = "line", chartOrientation = "vertical", selectedSingleCycle = false, showValues = false, showProductNames = false, labelSize = "medium", rotateProductNames = false }: { analytics: AdvancedSurveyAnalytics; scopeLabel: string; chartType?: ChartType; chartOrientation?: ChartOrientation; selectedSingleCycle?: boolean; showValues?: boolean; showProductNames?: boolean; labelSize?: ChartLabelSize; rotateProductNames?: boolean }) {
  ...
  useEffect(() => {
    Animated.timing(tooltipOpacity, { toValue: tooltip ? 1 : 0, duration: 180, useNativeDriver: true }).start();
  ...
  const chart = buildChart({ chartType, chartOrientation: effectiveOrientation, usesProductAxis, analytics, products, colors, width, chartHeight, padding, xAt, yAt, getValue, showValues, showProductNames: showProductNames || usesProductAxis, labelFontSize: LABEL_FONT_SIZES[labelSize], rotateProductNames, onBarInteraction: setTooltip, onBarHoverEnd: dismissTooltip });

  return <Pressable onPress={dismissTooltip} style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
...
type ChartBuildProps = { chartType: ChartType; chartOrientation: ChartOrientation; usesProductAxis: boolean; analytics: AdvancedSurveyAnalytics; products: CycleProductPresence[]; colors: Palette; width: number; chartHeight: number; padding: { left: number; right: number; top: number; bottom: number }; xAt: (index: number) => number; yAt: (value: number) => number; getValue: (productId: string, index: number) => number; showValues: boolean; showProductNames: boolean; labelFontSize: number; rotateProductNames: boolean; onBarInteraction: (tooltip: ChartTooltip) => void; onBarHoverEnd: () => void };

function buildChart({ chartType, chartOrientation, usesProductAxis, analytics, products, colors, width, chartHeight, padding, xAt, yAt, getValue, showValues, showProductNames, labelFontSize, rotateProductNames, onBarInteraction, onBarHoverEnd }: ChartBuildProps) {
  ...
  if (chartType === "bar") return chartOrientation === "horizontal" ? <HorizontalBarChart products={products} count={analytics.points.length} getValue={getValue} colors={colors} width={width} chartHeight={chartHeight} /> : <BarChart products={products} count={analytics.points.length} getValue={getValue} colors={colors} width={width} padding={padding} yAt={yAt} showValues={showValues} showProductNames={showProductNames} labelFontSize={labelFontSize} rotateProductNames={rotateProductNames} onBarInteraction={onBarInteraction} onBarHoverEnd={onBarHoverEnd} />;
...
function BarChart({ products, count, getValue, colors, width, padding, yAt, showValues, showProductNames, labelFontSize, rotateProductNames, onBarInteraction, onBarHoverEnd }: Pick<ChartBuildProps, "products" | "getValue" | "colors" | "width" | "padding" | "yAt" | "showValues" | "showProductNames" | "labelFontSize" | "rotateProductNames" | "onBarInteraction" | "onBarHoverEnd"> & { count: number }) {
  ...
  return products.flatMap((product, productIndex) => Array.from({ length: count }).map((_, index) => {
    const value = getValue(product.productId, index); const height = baseY - yAt(value); const flatIndex = index * products.length + productIndex; const x = padding.left + dynamicGap + flatIndex * (barWidth + dynamicGap); const renderedWidth = Math.max(1.5, barWidth); const center = x + renderedWidth / 2; const valueY = Math.max(padding.top + labelFontSize + 2, yAt(value) - 5); const nameY = baseY + (rotateProductNames ? 48 : 17); const tooltip = { title: toWesternDigits(product.productName), detail: `الدورة: ${index + 1}`, value, x: center, y: yAt(value) }; const webHoverProps = Platform.OS === "web" ? { onMouseEnter: () => onBarInteraction(tooltip), onMouseLeave: onBarHoverEnd } : {};
    return <G key={`${product.productId}-${index}`}><Rect x={x} y={yAt(value)} width={renderedWidth} height={height} rx={3} fill={product.color} onPress={(event) => { event.stopPropagation?.(); onBarInteraction(tooltip); }} {...(webHoverProps as any)} />{showValues ? <SvgText x={center} y={valueY} textAnchor="middle" fill={product.color} fontSize={labelFontSize + 1} fontWeight="700">{value}%</SvgText> : null}{showProductNames ? <SvgText x={center} y={nameY} textAnchor={rotateProductNames ? "end" : "middle"} transform={rotateProductNames ? `rotate(-45 ${center} ${nameY})` : undefined} fill={colors.foreground} fontSize={labelFontSize} fontWeight="700">{toWesternDigits(product.productName)}</SvgText> : null}</G>;
  }));
}
...
const styles = StyleSheet.create({ card: { borderRadius: 18, borderWidth: 1, padding: 14 }, title: { fontSize: 15, fontWeight: "800", textAlign: "right" }, caption: { fontSize: 11, textAlign: "right", marginTop: 4, marginBottom: 8 }, scrollContent: { paddingBottom: 4 }, chartCanvas: { position: "relative" }, tooltip: { position: "absolute", width: 154, minHeight: 42, borderRadius: 10, borderWidth: 1, paddingHorizontal: 9, paddingVertical: 6, alignItems: "flex-end", shadowColor: "#0F172A", shadowOpacity: 0.16, shadowRadius: 5, elevation: 4 }, tooltipTitle: { fontSize: 10, fontWeight: "800", textAlign: "right" }, tooltipDetail: { fontSize: 8, textAlign: "right", marginTop: 1 }, tooltipValue: { fontSize: 13, fontWeight: "900", textAlign: "right", marginTop: 1 }, labels: { flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 4 }, label: { width: 82, fontSize: 9, textAlign: "center", lineHeight: 13 }, productAxisLabel: { fontWeight: "700" }, horizontalLabels: { position: "absolute", top: 0, left: 0, bottom: 0 }, horizontalLabel: { position: "absolute", right: 0, fontSize: 9, lineHeight: 18, textAlign: "right", fontWeight: "700" }, legend: { flexDirection: "row", flexWrap: "wrap", gap: 9, marginTop: 8 }, legendItem: { flexDirection: "row", alignItems: "center", gap: 5 }, dot: { width: 9, height: 9, borderRadius: 5 }, legendText: { fontSize: 10 }, emptyCard: { borderRadius: 18, borderWidth: 1, padding: 22 }, emptyText: { textAlign: "center", lineHeight: 20, fontSize: 13 } });
