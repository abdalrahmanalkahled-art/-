...
export default function ReportsModule() {
  ...
  return <View style={styles.root}>
    ...
    <SelectionModal visible={panel === "templates"} colors={colors} onClose={() => setPanel(null)}>{<FlatList data={templates} keyExtractor={(item) => item.id} ListHeaderComponent={<TouchableOpacity onPress={() => void uploadTemplate()} style={[styles.generate, { backgroundColor: colors.primary }]}><Text style={styles.generateText}>رفع قالب جديد</Text></TouchableOpacity>} renderItem={({ item }) => <TouchableOpacity onPress={() => { setSelectedTemplateId(item.id); setPanel(null); }} style={[styles.tagRow, { borderColor: colors.border }]}><Text style={[styles.value, { color: colors.foreground }]}>{item.name}</Text></TouchableOpacity>} />}</SelectionModal>
    <SelectionModal visible={panel === "cycles"} colors={colors} onClose={() => setPanel(null)}>{<FlatList data={cycles} keyExtractor={(item) => item.id} renderItem={({ item }) => <TouchableOpacity onPress={() => { setSelectedCycleId(item.id); setPanel(null); }} style={[styles.tagRow, { borderColor: colors.border }]}><Text style={[styles.value, { color: colors.foreground }]}>{item.name}</Text></TouchableOpacity>} />}</SelectionModal>
...
function SelectionModal({ visible, colors, onClose, children }: { visible: boolean; colors: any; onClose: () => void; children: React.ReactNode }) { return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={styles.overlay}><Pressable style={StyleSheet.absoluteFill} onPress={onClose} /><View style={[styles.sheet, { backgroundColor: colors.surface }]}><TouchableOpacity onPress={onClose} style={styles.close}><MaterialIcons name="close" size={22} color={colors.muted} /></TouchableOpacity>{children}</View></View></Modal>; }
...
const styles = StyleSheet.create({
  root: { flex: 1 }, content: { padding: 12, gap: 10, paddingBottom: 104 }, hero: { borderRadius: 18, padding: 16, flexDirection: "row-reverse", gap: 12, alignItems: "flex-start" }, heroText: { flex: 1 }, heroTitle: { color: "#fff", fontSize: 18, fontWeight: "900", textAlign: "right" }, heroBody: { color: "#fff", fontSize: 12, opacity: 0.92, textAlign: "right", lineHeight: 18, marginTop: 3 }, sectionTitle: { fontWeight: "900", textAlign: "right", fontSize: 15, marginTop: 7 }, sectionLabel: { fontSize: 12, fontWeight: "900", textAlign: "right", marginTop: 17, marginBottom: 7 }, selector: { minHeight: 67, borderRadius: 15, borderWidth: 1, padding: 12, flexDirection: "row-reverse", gap: 10, alignItems: "center" }, selectorText: { flex: 1, alignItems: "flex-end" }, label: { fontSize: 11, textAlign: "right" }, value: { fontSize: 13, fontWeight: "800", textAlign: "right" }, generate: { minHeight: 49, borderRadius: 14, alignItems: "center", justifyContent: "center", flexDirection: "row-reverse", gap: 8, paddingHorizontal: 14 }, generateText: { color: "#fff", fontSize: 14, fontWeight: "900" }, disabled: { opacity: 0.6 }, templateRow: { minHeight: 57, borderWidth: 1, borderRadius: 13, flexDirection: "row-reverse", gap: 11, padding: 11, alignItems: "center" }, templateMain: { flex: 1, alignItems: "flex-end" }, empty: { borderWidth: 1, padding: 18, borderRadius: 13, alignItems: "center" }, fab: { position: "absolute", right: 18, bottom: 19, width: 58, height: 58, borderRadius: 29, alignItems: "center", justifyContent: "center", elevation: 5 }, fabMenu: { position: "absolute", right: 18, bottom: 87, minWidth: 220, borderRadius: 16, borderWidth: 1, paddingVertical: 6, elevation: 6 }, fabItem: { minHeight: 44, flexDirection: "row-reverse", alignItems: "center", gap: 9, paddingHorizontal: 13 }, fabItemText: { fontSize: 12, fontWeight: "800" }, overlay: { flex: 1, backgroundColor: "#00000066", justifyContent: "flex-end" }, floatingOverlay: { justifyContent: "center", alignItems: "center", padding: 18 }, sheet: { maxHeight: "82%", borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 18, paddingTop: 36 }, floatingSheet: { width: "100%", maxWidth: 500, maxHeight: "82%", borderRadius: 22, borderWidth: 1, padding: 18, paddingTop: 38 }, close: { position: "absolute", top: 11, left: 14, zIndex: 2 }, modalTitle: { fontSize: 17, fontWeight: "900", textAlign: "right", marginBottom: 6 }, modalBody: { fontSize: 12, textAlign: "right", lineHeight: 19, marginBottom: 12 }, settingsContent: { paddingBottom: 6 }, tagsButton: { minHeight: 49, borderWidth: 1, borderRadius: 13, paddingHorizontal: 12, flexDirection: "row-reverse", alignItems: "center", gap: 8, marginTop: 11 }, productTagsButton: { minHeight: 48, borderWidth: 1, borderRadius: 13, paddingHorizontal: 12, flexDirection: "row-reverse", alignItems: "center", gap: 8, marginTop: 12 }, tagsButtonText: { flex: 1, textAlign: "right", fontSize: 13, fontWeight: "800" }, search: { minHeight: 44, borderWidth: 1, borderRadius: 12, paddingHorizontal: 11, flexDirection: "row-reverse", alignItems: "center", gap: 8, marginBottom: 10 }, searchInput: { flex: 1, fontSize: 13, paddingVertical: 0 }, metricList: { maxHeight: 350 }, metricProduct: { minHeight: 50, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: "row-reverse", alignItems: "center", gap: 10, paddingHorizontal: 10 }, metricBadge: { width: 24, height: 24, borderRadius: 12, alignItems: "center", justifyContent: "center" }, metricBadgeText: { fontSize: 11, fontWeight: "800" as any }, emptySearch: { padding: 22, textAlign: "center", fontSize: 12 }, tagRow: { borderBottomWidth: 1, minHeight: 62, paddingVertical: 9, flexDirection: "row-reverse", gap: 9, alignItems: "center" }, imageChoice: { borderWidth: 1, borderRadius: 12, minHeight: 70, padding: 11, flexDirection: "row-reverse", gap: 10, alignItems: "center", marginBottom: 9 }, row: { flexDirection: "row-reverse", gap: 8, marginBottom: 14 }, choice: { flex: 1, borderWidth: 1, borderRadius: 11, minHeight: 42, alignItems: "center", justifyContent: "center" }, chips: { flexDirection: "row-reverse", gap: 7, marginTop: 8, paddingHorizontal: 1, paddingBottom: 2 }, chip: { borderWidth: 1, borderRadius: 18, paddingHorizontal: 10, paddingVertical: 6 }, chipText: { fontSize: 11, fontWeight: "800" },
});
