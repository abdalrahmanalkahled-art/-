...
import { getRecentSearches, loadGlobalSearchIndex, recordRecentSearch, searchGlobalIndex, type GlobalSearchResult, type RecentSearch, type SearchCategory } from "@/lib/global-search";
...
export default function SearchScreen() {
  ...
  useEffect(() => { void getRecentSearches().then(setRecentSearches); }, []);
  useEffect(() => {
    if (query.trim().length < 2) return;
    const timer = setTimeout(() => { void recordRecentSearch(query).then(setRecentSearches); }, 700);
...
const styles = StyleSheet.create({
  ...
  body: { flex: 1 }, searchBox: { margin: 14, minHeight: 54, borderWidth: 1, borderRadius: 17, paddingHorizontal: 14, flexDirection: "row", alignItems: "center", gap: 10 }, input: { flex: 1, fontSize: 14, minHeight: 48 }, filtersList: { height: 42, minHeight: 42, maxHeight: 42, flexGrow: 0, flexShrink: 0, marginBottom: 0 }, filters: { height: 42, paddingHorizontal: 14, gap: 8, alignItems: "center", flexDirection: "row-reverse" }, filter: { height: 34, borderRadius: 11, borderWidth: 1, paddingHorizontal: 12, alignItems: "center", justifyContent: "center" }, filterText: { fontSize: 12, fontWeight: "800" as const },
  resultsList: { flex: 1, marginTop: 0 }, results: { padding: 14, paddingTop: 0, paddingBottom: 34, gap: 8 }, result: { minHeight: 76, borderRadius: 16, borderWidth: 1, padding: 12, flexDirection: "row", alignItems: "center", gap: 10 }, resultIcon: { width: 40, height: 40, borderRadius: 13, alignItems: "center", justifyContent: "center" }, resultCopy: { flex: 1, alignItems: "flex-end" }, titleLine: { flexDirection: "row", alignItems: "center", width: "100%", justifyContent: "space-between", gap: 9 }, resultTitle: { flex: 1, fontSize: 14, fontWeight: "800" as const, textAlign: "right" }, moduleLabel: { fontSize: 10, fontWeight: "800" as const, textAlign: "left" }, resultSubtitle: { width: "100%", fontSize: 11, lineHeight: 17, textAlign: "right", marginTop: 4 },
  empty: { borderWidth: 1, borderRadius: 18, padding: 25, alignItems: "center", marginTop: 16 }, emptyIcon: { width: 58, height: 58, borderRadius: 18, alignItems: "center", justifyContent: "center" }, emptyTitle: { fontSize: 15, fontWeight: "800" as const, marginTop: 13 }, emptyText: { fontSize: 11, lineHeight: 18, textAlign: "center", marginTop: 6 }, loader: { marginTop: 35 }, recentCard: { borderWidth: 1, borderRadius: 16, overflow: "hidden", marginTop: 10 }, recentTitle: { fontSize: 13, fontWeight: "800" as const, textAlign: "right", padding: 14 }, recentRow: { minHeight: 46, paddingHorizontal: 14, borderTopWidth: StyleSheet.hairlineWidth, flexDirection: "row", alignItems: "center", gap: 9 }, recentText: { flex: 1, textAlign: "right", fontSize: 12 },
});
