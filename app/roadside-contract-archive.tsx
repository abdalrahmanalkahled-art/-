...
export default function RoadsideContractArchiveScreen() {
  ...
  return <ScreenContainer containerClassName="bg-background">
    <View style={[styles.header, { borderBottomColor: colors.border }]}><TouchableOpacity onPress={() => router.back()} style={[styles.back, { backgroundColor: colors.surface }]}><MaterialIcons name="arrow-forward" size={21} color={colors.foreground} /></TouchableOpacity><View style={styles.headerCopy}><Text style={[styles.title, { color: colors.foreground }]}>أرشيف عقود اللوحات</Text><Text style={[styles.subtitle, { color: colors.muted }]}>العقود الملغاة والمجددة محفوظة للمراجعة</Text></View><View style={styles.headerActions}><TouchableOpacity disabled={Boolean(exporting)} onPress={() => void exportArchiveReport("pdf")} accessibilityLabel="تصدير أرشيف العقود PDF" style={[styles.reportAction, { backgroundColor: "#DC262616", opacity: exporting && exporting !== "pdf" ? 0.45 : 1 }]}><MaterialIcons name="picture-as-pdf" size={19} color="#DC2626" /></TouchableOpacity><TouchableOpacity disabled={Boolean(exporting)} onPress={() => void exportArchiveReport("excel")} accessibilityLabel="تصدير أرشيف العقود Excel" style={[styles.reportAction, { backgroundColor: "#21734616", opacity: exporting && exporting !== "excel" ? 0.45 : 1 }]}><MaterialIcons name="table-chart" size={19} color="#217346" /></TouchableOpacity><TouchableOpacity onPress={() => setReportSettingsOpen(true)} accessibilityLabel="إعدادات تقرير الأرشيف" style={[styles.reportAction, { backgroundColor: colors.primary + "12" }]}><MaterialIcons name="tune" size={19} color={colors.primary} /></TouchableOpacity></View></View>
...
const styles = StyleSheet.create({
  header: { minHeight: 73, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", gap: 9, borderBottomWidth: StyleSheet.hairlineWidth }, back: { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center" }, headerCopy: { flex: 1, alignItems: "flex-end" }, headerActions: { flexDirection: "row", gap: 5, alignItems: "center" }, reportAction: { width: 33, height: 33, borderRadius: 11, alignItems: "center", justifyContent: "center" }, title: { fontSize: 17, fontWeight: "800" as any }, subtitle: { fontSize: 10, marginTop: 3 }, typeTabs: { flexDirection: "row-reverse", gap: 8, paddingHorizontal: 16, paddingTop: 14, paddingBottom: 9 }, typeTab: { flex: 1, minHeight: 39, borderRadius: 11, borderWidth: 1, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 5 }, typeText: { fontSize: 11, fontWeight: "800" as any }, list: { padding: 16, gap: 9, paddingBottom: 28 }, emptyList: { flexGrow: 1, justifyContent: "center", padding: 24 }, card: { minHeight: 96, borderWidth: 1, borderRadius: 16, padding: 13, flexDirection: "row-reverse", gap: 10, alignItems: "center" }, icon: { width: 46, height: 46, borderRadius: 15, alignItems: "center", justifyContent: "center" }, copy: { flex: 1, alignItems: "flex-end" }, cardTitleRow: { width: "100%", flexDirection: "row-reverse", alignItems: "center", justifyContent: "space-between", gap: 8 }, cardTitle: { flex: 1, fontSize: 13, fontWeight: "800" as any, textAlign: "right" }, status: { borderRadius: 8, paddingHorizontal: 7, paddingVertical: 3 }, statusText: { fontSize: 9, fontWeight: "800" as any }, cardMeta: { marginTop: 5, fontSize: 10, textAlign: "right" }, cardHint: { marginTop: 4, fontSize: 9, textAlign: "right" }, empty: { alignItems: "center", gap: 8 }, emptyTitle: { fontSize: 15, fontWeight: "800" as any }, emptyText: { maxWidth: 260, textAlign: "center", fontSize: 11, lineHeight: 18 },
});
