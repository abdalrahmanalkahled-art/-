...
async function buildDailyReportHtml(report: DailyReportData, options: DailyReportExportOptions = {}): Promise<string> {
  ...
  const eventHtml = report.events.length ? `<section class=\"events\"><h2>الفعاليات المنفذة</h2>${report.events.map((event) => `<div class=\"event\"><b>${escapeHtml(event.title || "فعالية")}</b><span>${formatDate(event.eventDate || event.createdAt)}${event.location ? ` • ${escapeHtml(event.location)}` : ""}</span>${event.description ? `<p>${escapeHtml(event.description)}</p>` : ""}</div>`).join("")}</section>` : "";
