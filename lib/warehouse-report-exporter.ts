...
function reportHtml(data: WarehouseReportData, settings: WarehouseReportSettings, logoMarkup = ""): string {
  const summary = [
  ...
  const tools = settings.includeTools ? `<section><h2>الأدوات</h2>${table([...selected(settings.toolColumns, TOOL_FIELDS, {}).columns, ...(settings.includeImages ? ["الصورة"] : [])], data.tools.map((tool) => [...settings.toolColumns.map((column) => escapeHtml({ name: tool.name, quantity: tool.quantity, brands: tool.brandNames.join("، ") || "—", condition: conditionMeta(tool.condition).label }[column])), ...(settings.includeImages ? [imageCell(tool.imageUri, true)] : [])]))}</section>` : "";
