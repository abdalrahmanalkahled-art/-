...
async function toolCards(data: WarehouseReportData, settings: WarehouseReportSettings, onImagePrepared?: () => void): Promise<string> {
  ...
  for (const [index, tool] of data.tools.entries()) {
    ...
    if (settings.includeImages && tool.imageUri) onImagePrepared?.();
...
async function reportHtml(data: WarehouseReportData, settings: WarehouseReportSettings, logoMarkup = "", onImagePrepared?: () => void): Promise<string> {
  const summary = [
  ...
  if (settings.includeTools && settings.toolDisplayMode !== "cards") {
    for (const tool of data.tools) {
      ...
      if (settings.includeImages) {
        row.push(await embeddedToolImage(tool.imageUri, settings));
        if (tool.imageUri) onImagePrepared?.();
      }
      toolRows.push(row);
    }
  }
  const tools = settings.includeTools ? `<section><h2>الأدوات</h2>${settings.toolDisplayMode === "cards" ? await toolCards(data, settings, onImagePrepared) : table([...selected(settings.toolColumns, TOOL_FIELDS, {}).columns, ...(settings.includeImages ? ["الصورة"] : [])], toolRows)}</section>` : "";
...
export async function exportWarehouseReport(format: "pdf" | "excel", data: WarehouseReportData, settings: WarehouseReportSettings): Promise<void> { if (!data.materials.length && !data.tools.length && !data.movements.length) { Alert.alert("تنبيه", "لا توجد بيانات ضمن خيارات التقرير المختارة."); return; } if (Platform.OS === "web") { Alert.alert("التصدير من الويب", "تصدير تقارير المستودع متاح في تطبيق الهاتف."); return; } const imageCount = settings.includeImages ? data.tools.filter((tool) => Boolean(tool.imageUri)).length : 0; const progress = beginOperationProgress({ kind: "export", title: `تصدير تقرير المستودع ${format === "pdf" ? "PDF" : "Excel"}`, steps: ["تجهيز بيانات التقرير", "تجهيز صور الأدوات", format === "pdf" ? "إنشاء ملف PDF" : "بناء مصنف Excel", "حفظ التقرير والتحقق منه", "فتح المشاركة"] }); try { progress.update({ stepIndex: 0, message: "جارٍ تجهيز بيانات المستودع" }); if (format === "pdf") { let preparedImages = 0; progress.update({ stepIndex: 1, message: imageCount ? "جارٍ تجهيز صور الأدوات" : "لا توجد صور أدوات لتجهيزها", completedItems: 0, totalItems: imageCount }); const html = await reportHtml(data, settings, pdfLogoMarkup(await loadSharedPdfReportLogo()), () => { preparedImages += 1; progress.update({ stepIndex: 1, message: `جارٍ تجهيز صورة الأداة ${preparedImages} من ${imageCount}`, completedItems: preparedImages, totalItems: imageCount }); }); progress.update({ stepIndex: 2, message: "جارٍ إنشاء ملف PDF" }); const generated = await Print.printToFileAsync({ html }); progress.update({ stepIndex: 3, message: "جارٍ حفظ التقرير والتحقق من الملف" }); const { uri, filename } = await destination("pdf"); await FileSystem.copyAsync({ from: generated.uri, to: uri }); progress.update({ stepIndex: 4, message: "جارٍ فتح خيارات مشاركة التقرير" }); await shareAndRecord(uri, filename, "PDF"); } else { progress.update({ stepIndex: 2, message: "جارٍ بناء مصنف Excel" }); const base64 = workbookBase64(data, settings); progress.update({ stepIndex: 3, message: "جارٍ حفظ التقرير والتحقق من الملف" }); const { uri, filename } = await destination("xlsx"); await FileSystem.writeAsStringAsync(uri, base64, { encoding: FileSystem.EncodingType.Base64 }); progress.update({ stepIndex: 4, message: "جارٍ فتح خيارات مشاركة التقرير" }); await shareAndRecord(uri, filename, "XLSX"); } } catch (error) { Alert.alert(`فشل تصدير ${format === "pdf" ? "PDF" : "Excel"}`, format === "pdf" ? pdfExportErrorMessage(error) : error instanceof Error ? error.message : "خطأ غير معروف"); } finally { progress.complete(); } }
