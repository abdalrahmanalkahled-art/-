...
const DATA_DOMAINS: DataDomain[] = [
  ...
  { id: "plan", title: "الخطة التسويقية والأهداف والفعاليات", keywords: ["خطة", "هدف", "أهداف", "تقدم", "kpi", "انتهاء", "منتهي", "مستهدف", "إنجاز", "فعالية", "فعاليات"], keys: [STORAGE_KEYS.MARKETING_GOALS, STORAGE_KEYS.EVENTS, STORAGE_KEYS.BRANDS, STORAGE_KEYS.REGIONS], fields: {
    [STORAGE_KEYS.MARKETING_GOALS]: ["id", "title", "name", "description", "brandName", "region", "startDate", "endDate", "period", "kpi", "targetValue", "currentValue", "completionPercentage", "status", "createdAt", "updatedAt"], [STORAGE_KEYS.EVENTS]: ["id", "title", "eventDate", "region", "brandName", "status", "beneficiariesCount", "goalId", "createdAt", "updatedAt"], [STORAGE_KEYS.BRANDS]: ["id", "name", "isActive"], [STORAGE_KEYS.REGIONS]: ["id", "name", "isActive"],
  } },
...
export async function buildSmartDataContext(question: string, selectedScopeIds: string[]): Promise<{ context: string; domainIds: string[]; freshness: string }> {
  ...
  domains.forEach((domain) => {
    ...
    if (domain.id === "field") {
      ...
      payload["الأهداف المرتبطة"] = sortFresh(selectRelevantRecords(relatedGoals, question)).map((goal) => compactValue(goal, ["id", "title", "description", "brandName", "startDate", "endDate", "period", "kpi", "targetValue", "currentValue", "completionPercentage", "status", "createdAt", "updatedAt"]));
    }
